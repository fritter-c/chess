...
namespace renderer
{
    ...
    VisualBoard::load_textures(const std::filesystem::path &res_path)
    {
        ...
        if (ma_sound_init_from_file(&sound_engine, move.c_str(), MA_SOUND_FLAG_DECODE, nullptr, nullptr, &move_sound) != MA_SUCCESS)
        {
            return false;
        }
        const auto check = res_path / "check.mp3";
        if (ma_sound_init_from_file(&sound_engine, check.c_str(), MA_SOUND_FLAG_DECODE, nullptr, nullptr, &check_sound) != MA_SUCCESS)
        {
            return false;
        }
    ...
    render_available_squares(const VisualBoard *board)
    {
        for (int8_t i = 0; i < 64; ++i)
        {
            if (board->available_squares_for_dragging.get(i))
            {
                ...
                Rectangle cell_rect;
                visual_board_get_rect_for_cell(board, row, col, cell_rect);
    ...
    render_chess_board(game::Game *game, VisualBoard *board)
    {
        ...
        for (int32_t rank = 7; rank >= 0; --rank)
        {
            ...
            for (int32_t file = 0; file < 8; ++file)
            {
                ...
                float font_size = board->cell_size > 60.0f ? 20.0f : board->cell_size / 3.0f;
                draw_list->AddText(ImGui::GetFont(),font_size, cell_pos, IM_COL32(255, 255, 255, 255), name);

                // hit-testing / clicks
                ImGui::PushID(game::Board::board_get_index(flipped_rank, flipped_file));
                ImRect cell_rect(
                    cell_pos,
                    ImVec2(cell_pos.x + board->cell_size,
                           cell_pos.y + board->cell_size));
                ...
                {
                    auto index = game::Board::board_get_index(flipped_rank, flipped_file);
                    if (game->board.pieces[index] != game::PIECE_NONE)
                    {
                        board->dragging_piece_index = index;
                        board->available_squares_for_dragging = game::analyzer_get_available_moves_for_piece(&game->board, index);
                ...
                else if (board->dragging_piece_index != -1 && ImGui::IsMouseReleased(ImGuiMouseButton_Left) && ImGui::IsItemHovered())
                {
                    ...
                    if (game->move(game::Board::board_get_row(board->dragging_piece_index), game::Board::board_get_col(board->dragging_piece_index),
