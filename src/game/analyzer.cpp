...
namespace game {
            ...
            while (r >= 0 && r < 8 && c >= 0 && c < 8) {
                ...
                if (const bool is_diag = (dr != 0 && dc != 0);
                    PIECE_COLOR(p) != attacker || !(
                        PIECE_TYPE(p) == QUEEN || (PIECE_TYPE(p) == ROOK && !is_diag) || (
                            PIECE_TYPE(p) == BISHOP && is_diag))) {
                    break;
                }
                return true;
            }
    ...
    analyzer_get_pawn_moves(const Board *board, const Piece piece, const int32_t row, const int32_t col,
        ...
        for (const int32_t dc: {-1, +1}) {
            ...
            if (const auto p = board->pieces[Board::board_get_index(r2, c2)];
                PIECE_TYPE(p) != EMPTY && PIECE_COLOR(p) == enemy) {
    ...
    analyzer_get_king_moves(const Board *board, const Piece piece, const int32_t row, const int32_t col,
        ...
        if (!analyzer_is_color_in_check(board, friendly)) {
            ...
            if (PIECE_TYPE(board->pieces[Board::board_get_index(king_row, 5)]) == EMPTY &&
                PIECE_TYPE(board->pieces[Board::board_get_index(king_row, 6)]) == EMPTY &&
            ...
            // Queenside castling
            if (PIECE_TYPE(board->pieces[Board::board_get_index(king_row, 3)]) == EMPTY &&
                PIECE_TYPE(board->pieces[Board::board_get_index(king_row, 2)]) == EMPTY &&
                PIECE_TYPE(board->pieces[Board::board_get_index(king_row, 1)]) == EMPTY &&
    ...
    analyzer_get_sliders_moves(const Board *board, const Piece piece, const int32_t row, const int32_t col,
        ...
        for (auto [dr, dc]: KING_DELTAS) {
            ...
            auto is_enemy_next = [&r, &c, &board, &enemy]() {
                return PIECE_TYPE(board->pieces[Board::board_get_index(r, c)]) != EMPTY && PIECE_COLOR(
    ...
    analyzer_get_available_moves_for_piece(const Board *board, const int32_t row, const int32_t col) {
        ...
        if (const auto piece = board->pieces[Board::board_get_index(row, col)]; PIECE_TYPE(piece) != EMPTY) {
