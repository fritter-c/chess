...
namespace game {
    ...
    bool
    Game::move(const int32_t from_row, const int32_t from_col, const int32_t to_row, const int32_t to_col) {
        if (!game_is_playable(this)) {
            return false;
        }

        const Color moving_color = board.get_color(from_row, from_col);
        if (moving_color == turn && analyzer_can_move(&board, from_row, from_col, to_row, to_col)) {
            const Move move = analyzer_get_move_from_simple(&board, SimpleMove{
    ...
    Game::move(const Move &move) {
        ...
        if (moving_color == turn && analyzer_can_move(&board, move.from_row(), move.from_col(), move.to_row(),
                                                      move.to_col()) && board.board_move(move)) {
            turn = ~turn;
            game_update_status(this);
            ++move_count;
            return true;
